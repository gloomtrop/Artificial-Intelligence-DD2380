...
(:action have-lunch
	...
	:parameters ( ?x ?y ?g)
	:precondition (and (PERSON ?x) (IS-IN-GROUP ?x ?g) (GROUP ?g) (IS-GROUP-AT ?g ?y) 
	                (is-person-at ?x ?y) (ROOM ?y) (HAS-CATERING ?y)
	                (not (had-lunch ?x)))
...
(:action attend-morning-talk
	; WRITE HERE THE CODE FOR THIS ACTION
	:parameters (?x ?y ?z ?w)
	:precondition (and (PERSON ?x) (IS-ATTENDEE ?x) (PERSON ?y) (IS-SPEAKER ?y) (IS-MORNING ?w) (ROOM ?z) (TALK ?w) (IS-TALK-AT ?w ?z) 
	                (is-person-at ?x ?z) (is-person-at ?y ?z) (gives-talk ?y ?w)
	                (not (had-lunch ?x) ))
	:effect (and (attended-talk ?x ?w))               
)
...
(:action attend-afternoon-talk
	; WRITE HERE THE CODE FOR THIS ACTION
	:parameters (?x ?y ?z ?w)
	:precondition (and (PERSON ?x) (IS-ATTENDEE ?x) (PERSON ?y) (IS-SPEAKER ?y) (IS-AFTERNOON ?w) (ROOM ?z) (TALK ?w) (IS-TALK-AT ?w ?z) 
	                (is-person-at ?x ?z) (is-person-at ?y ?z) (gives-talk ?y ?w)
	                (had-lunch ?x))
	:effect (and (attended-talk ?x ?w))
)
